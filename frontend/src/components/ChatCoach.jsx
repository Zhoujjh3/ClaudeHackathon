...
export default function ChatCoach({ context, initialMessage, onInitialMessageSent }) {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content:
        "I'm FieldFit Coach — built for journalists in the field. Tell me where you are, what's available to eat, or send a photo of a menu, your fridge, or your plate. I'll give you immediate, practical advice.\n\nNo meal prep. No nonsense. Just what to do right now.",
    },
  ])
  ...
  const sendMessage = useCallback(
    async (text, imageData) => {
        ...
        userApiContent = [
          {
            ...
            source: {
              ...
              data: imageData.preview.split(',')[1],
            },
          },
      ...
      // Display message (with image preview if present)
      const displayMsg = {
        role: 'user',
      ...
      // Build history for API — convert previous messages to text only
      const history = messages.map((m) => ({
        role: m.role,
        content:
          m.imagePreview
            ? `[User shared a food photo] ${m.content}`
            : m.content,
      }))
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            role: 'assistant',
            content:
              'Connection issue — make sure the backend is running (`uvicorn main:app --reload` in /backend), then try again.',
          },
        ])
      } finally {
        setLoading(false)
  ...
  // Auto-send scenario messages from dashboard
  useEffect(() => {
    if (initialMessage) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {(context.location || (context.energyLevel && context.energyLevel !== 'normal')) && (
        <div className="px-4 py-2 bg-[#0a160a] border-b border-green-900/30 flex gap-3 text-xs text-green-500 flex-shrink-0">
          {context.location && (
            ...
            >
              {msg.imagePreview && (
                <img
                  ...
                  alt="Food"
                  className="rounded-xl mb-2.5 max-h-52 object-cover w-full"
                />
              )}
              {msg.role === 'assistant' ? (
